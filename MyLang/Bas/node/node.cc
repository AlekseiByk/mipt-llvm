...
CodeGenCtx::CodeGenCtx() {
    ...
    llvm::Function::Create(functTy, llvm::Function::ExternalLinkage, "__win_print", *m_module);

    functTy = llvm::FunctionType::get(m_builder->getInt32Ty(), false);
    // __win_scan
    llvm::Function::Create(functTy, llvm::Function::ExternalLinkage, "__win_scan", *m_module);
}
...
void CodeGenCtx::initGraphics(std::shared_ptr<ScopeN> globalScope) {
    ...
    // __win_rand
    std::string name = "__win_rand";
    auto&& decl = std::make_shared<win::FuncDeclN>(name);
    ...
    // __win_flush
    name = "__win_flush";
    decl = std::make_shared<win::FuncDeclN>(name);
...
llvm::Value* UnOpN::codegen(CodeGenCtx& ctx) {
    ...
    switch (m_op) {
        ...
        case UnOp::Output:
        {
            auto* winPrint = module->getFunction("__win_print");
            assert(winPrint && "Driver shall create decl for __win_print");

            llvm::Value* args[] = { valCodegen };
            return builder->CreateCall(winPrint, args);
        }
        case UnOp::Input:
        {
            auto* winScan = module->getFunction("__win_scan");
            assert(winScan && "Driver shall create decl for __win_scan");

            return builder->CreateCall(winScan);
        }
    };
    assert(0);
    nullptr;
